      ...
      codeExamples: [
        {
          ...
          description:
            "Every element here is intentional — nothing is optional.",
          html: `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  <link rel="icon" href="/favicon.ico" />
      ...
      practiceTasks: [
        {
          description:
            "Write a valid HTML5 boilerplate from memory without looking it up. Include charset, viewport, title, and one OG meta tag.",
          hint: 'Start with <!DOCTYPE html>, then <html lang="en">, then <head> with at least 3 meta tags.',
        },
        {
          description:
            "Open any website in DevTools, find the <head>, and identify which meta tags are present. Note any you didn't recognize.",
  ...
  challenges: [
    {
      ...
      targetImage: {
        src: "/wf-challenges/semantic-blog-post.svg",
        alt: "Blog article mock with breadcrumbs, article header metadata, hero image, reading column, related links aside, and footer.",
        caption:
          "Use the mock to identify semantic regions before you write HTML: site header, breadcrumbs, main article, supporting aside, figure, and footer.",
      },
      requirements: [
...
export const explanations: Record<string, () => React.ReactNode> = {
  "document-structure": () => (
    <>
      <p>
        An HTML document is just a text file — but a text file the browser
        interprets according to a set of rules. The{" "}
        <code>{"<!DOCTYPE html>"}</code> declaration at the very first line is a
        short instruction telling the browser which rules to apply. Without it,
        browsers fall into <strong>quirks mode</strong>, a compatibility layer
        that emulates ancient, inconsistent behaviour from the 1990s. With it,
        you get <strong>standards mode</strong> — predictable rendering across
        browsers.
      </p>
      <p>
        The document is split into two regions with fundamentally different
        purposes. <code>{"<head>"}</code> contains{" "}
        <em>machine-readable metadata</em> — information that browsers and
        crawlers consume but users never directly see. This includes character
        encoding, viewport instructions, the page title (shown in the browser
        tab and by search engines), stylesheets, and social-sharing meta tags.{" "}
        <code>{"<body>"}</code> contains <em>human-readable content</em> —
        everything rendered on screen.
      </p>
      <p>
        Two meta tags are non-negotiable in every document.{" "}
        <code>{'<meta charset="UTF-8">'}</code> must appear within the first
        1024 bytes of the document — browsers begin parsing before the full file
        is received, and they need to know the encoding early to decode the rest
        correctly. The{" "}
        <code>
          {
            '<meta name="viewport" content="width=device-width, initial-scale=1.0">'
        ...
        tells mobile browsers to use the device&apos;s actual pixel width rather
        than rendering at ~980px and scaling down. Without it, your responsive
        CSS is ignored.
      </p>
    </>
  ),
  ...
  "semantic-html": () => (
    <>
      <p>
        When every element on a page is a <code>{"<div>"}</code>, the browser
        (and anyone reading the HTML) can&apos;t tell a navigation bar from an
        article from a sidebar. This is called <em>div-soup</em>. Semantic HTML
        replaces meaningless containers with elements whose names describe their
        purpose.
      </p>
      <p>
        The biggest benefit is not visual — it&apos;s structural. Screen readers
        let users jump between landmarks like navigation, main content, and
        complementary content. Search engines use these signals to understand
        which content is primary. And other developers can scan the HTML and
        understand the page without reverse-engineering your CSS classes.
      </p>
      <p>
        ...
        <code>{"<main>"}</code> holds the page&apos;s primary content and should
        appear only once per page. <code>{"<aside>"}</code> is for content
        related to, but not central to, the main flow — think related articles,
        ads, side notes, or filters. <code>{"<footer>"}</code> contains closing
        or meta information about its parent.
      </p>
      <p>
        For content structure, the most commonly confused pair is{" "}
        <code>{"<article>"}</code> vs <code>{"<section>"}</code>. Use{" "}
        <code>{"<article>"}</code> for a self-contained unit that would still
        make sense if copied somewhere else: a blog post, news card, comment,
        review, or forum thread. Use <code>{"<section>"}</code> for a thematic
        chapter within a larger page or article. If the content is merely a
        styling wrapper with no independent meaning, it probably should stay a{" "}
        <code>{"<div>"}</code>.
      </p>
      <p>
        A good rule of thumb is: semantics describe <em>purpose</em>, not
        appearance. A hero banner is not automatically a{" "}
        <code>{"<section>"}</code> just because it is a big visual block. A card
        is not automatically an <code>{"<article>"}</code> just because it looks
        like one. Ask what the content <em>is</em>. If it is a self-contained
        story, item, or post, <code>{"<article>"}</code> fits. If it is a
        chapter inside a bigger document, <code>{"<section>"}</code> fits. If it
        is just a flexbox wrapper, use <code>{"<div>"}</code>.
      </p>
      <p>
        Heading hierarchy is part of semantic HTML too. Headings are not just
        bigger text; they define the content outline. The page gets one primary{" "}
        <code>{"<h1>"}</code>, then subsections use <code>{"<h2>"}</code>, and
        nested subsections use <code>{"<h3>"}</code>, and so on. Skipping levels
        (for example,
        <code>h1</code> straight to <code>h3</code>) creates a confusing
        structure for assistive technology. The browser will still render it,
        but the document meaning becomes weaker.
      </p>
      <p>
        One subtle but important point: many semantic elements already expose
        implicit ARIA roles. <code>{"<nav>"}</code> is already a navigation
        landmark. <code>{"<main>"}</code> is already the main landmark.{" "}
        <code>{"<header>"}</code> and <code>{"<footer>"}</code> can map to
        landmark roles depending on context. That means adding{" "}
        <code>role="navigation"</code> to <code>{"<nav>"}</code> is usually
        redundant. Native semantics should be your first choice; ARIA should
        only fill gaps when HTML has no element that expresses the meaning you
        need.
      </p>
    </>
  ),
  ...
  "links-and-images": () => (
    <>
      <p>
        The <code>{"<a>"}</code> element&apos;s <code>href</code> attribute can
        hold any URL — including relative paths (<code>href="/about"</code>),
        anchor links (<code>href="#section-id"</code>), mailto links (
        <code>href="mailto:..."</code>), and tel links. When opening in a new
        tab with <code>{'target="_blank"'}</code>, always add{" "}
        <code>{'rel="noopener noreferrer"'}</code>: <code>noopener</code>{" "}
        prevents the opened page from accessing <code>window.opener</code>, and{" "}
        <code>noreferrer</code> also prevents the referrer header from being
        sent.
      </p>
      <p>
        The <code>alt</code> attribute on <code>{"<img>"}</code> is one of the
        most misunderstood attributes in HTML. It serves two purposes: it&apos;s
        the text spoken by screen readers when the image is encountered, and
        it&apos;s the text shown when the image fails to load. For{" "}
        <em>informational</em> images, it should describe what the image shows
        (not &quot;image of...&quot; or &quot;photo of...&quot;). For{" "}
        <em>decorative</em> images — backgrounds, dividers, purely visual
        flourishes — it should be an empty string (<code>alt=""</code>) to tell
        screen readers to skip it entirely. Omitting <code>alt</code> is always
        wrong.
      </p>
      <p>
        Performance matters too. Always specify <code>width</code> and{" "}
        <code>height</code> on images so the browser can reserve space before
        the image downloads, preventing
        <em>Cumulative Layout Shift</em> (CLS). Use{" "}
        <code>{'loading="lazy"'}</code> on images below the fold, and{" "}
        <code>{'loading="eager"'}</code> (or omit it) for above-the-fold images
        like hero shots. For high-resolution displays and responsive layouts,{" "}
        <code>srcset</code> and <code>sizes</code> let the browser download the
  ...
  "lists-and-tables": () => (
    <>
      <p>
        Lists and tables are some of the clearest examples of HTML carrying
        meaning beyond appearance. A list tells the browser that several items
        belong to the same set. A table tells it something even richer: values
        are related across rows and columns. If you collapse either into generic
        containers, the visual layout might survive, but the information
        architecture does not.
      </p>
      <p>
        Use <code>{"<ul>"}</code> when order does not matter,{" "}
        <code>{"<ol>"}</code> when sequence matters, and <code>{"<dl>"}</code>{" "}
        when a term is paired with a value or explanation. These are not just
        styling defaults for bullets and numbers. Screen readers announce list
        boundaries and item counts, which makes long content much easier to
        understand.
      </p>
      <p>
        Tables should be reserved for genuine tabular data: schedules,
        analytics, financial values, comparisons, and any content where headers
        change the meaning of each cell. Good table markup starts with a{" "}
        <code>{"<caption>"}</code> and then uses real header cells so a screen
        reader can announce both the value and the row or column context around
        it. That is why <code>{'<th scope="col">'}</code> and{" "}
        <code>{'<th scope="row">'}</code> matter.
      </p>
      <p>
        Modern CSS removed the old excuse for layout tables. Flexbox and grid
        are for composition; tables are for data. Being able to explain that
        distinction clearly is a very strong interview signal because it shows
        you care about meaning, not just visual output.
      </p>
    </>
  ),
};
